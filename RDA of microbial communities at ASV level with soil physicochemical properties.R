...
perform_rda <- function(community_data, soil_data, title, 
  ...
  # Add species labels (ASV names)
  p <- p +
    geom_point(data = subset(as.data.frame(rda_scores$species), rownames(rda_scores$species) %in% rownames(rda_scores$species)[1:10]), 
               aes(x = RDA1, y = RDA2), 
               size = 3, color = "#FFB55F", shape = 17) +
    geom_text_repel(data = subset(as.data.frame(rda_scores$species), rownames(rda_scores$species) %in% rownames(rda_scores$species)[1:10]), 
                    aes(x = RDA1, y = RDA2, label = rownames(subset(as.data.frame(rda_scores$species), rownames(rda_scores$species) %in% rownames(rda_scores$species)[1:10]))),
...
export_rda_results_csv <- function(rda_result, soil_data, title, dir_prefix) {
  dir.create(dir_prefix, showWarnings = FALSE)
  
  # Export environmental factor contributions
...
process_microbial_data <- function(microbial_data, soil_data, title) {
  ...
  common_samples <- intersect(colnames(microbial_data), rownames(soil_data))
  data_filtered <- microbial_data[, common_samples]
  soil_filtered <- soil_data[common_samples, ]
  
  # Perform RDA analysis
  rda_result <- perform_rda(data_filtered, soil_filtered, title, 
                            label_distance = 0.8, label_size = 3.5, max_overlaps = 20)
  
  # Export results
  export_rda_results_csv(rda_result, soil_filtered, title, paste0(title, "_rda_results"))
...
all_microbes <- rbind(
  data.frame(ASV = rownames(archaea), Domain = "Archaea", archaea),
  data.frame(ASV = rownames(bacteria), Domain = "Bacteria", bacteria),
  data.frame(ASV = rownames(eukaryote), Domain = "Eukaryote", eukaryote),
  data.frame(ASV = rownames(fungi), Domain = "Fungi", fungi)
)
