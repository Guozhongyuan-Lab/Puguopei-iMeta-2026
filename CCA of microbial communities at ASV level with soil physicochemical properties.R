...
perform_cca <- function(community_data, soil_data, title, 
...
export_cca_results_csv <- function(cca_result, soil_data, title, dir_prefix) {
  ...
  # 2. Export species scores (with ASV IDs)
...
process_microbial_data <- function(microbial_data, soil_data, title) {
  ...
  cca_result <- perform_cca(data_filtered, soil_filtered, title, 
                            label_distance = 0.8, label_size = 3.5, max_overlaps = 20)
  
  # Export results
  export_cca_results_csv(cca_result, soil_filtered, title, paste0(title, "_cca_results"))
